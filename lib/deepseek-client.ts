/**
 * 前端直接调用 DeepSeek API
 * 用于 Vercel 部署，无需后端服务器
 */

const DEEPSEEK_API_BASE = "https://api.deepseek.com";

// 从环境变量获取 API Key（Vercel 会注入 EXPO_PUBLIC_ 前缀的环境变量到前端）
const getApiKey = () => {
  // 优先使用 EXPO_PUBLIC_ 前缀的环境变量（Vercel 前端可访问）
...
function extractKeywords(input: string): string[] {
  ...
  // 常见动词关键词
  const verbs = ["洗澡", "批评", "考试", "加班", "学习", "工作", "跑步", "做饭", "打扫", "照顾", "陪伴", "完成", "坚持", "努力", "挑战", "克服", "面对", "接受", "处理", "解决", "写", "读", "看", "听", "说", "吃", "喝", "睡", "玩", "买", "卖", "送", "收", "开", "关", "走", "跑", "飞", "游", "爬", "跳", "哭", "笑", "累", "困", "饿", "渴", "冷", "热", "忙", "闲"];
  
  // 常见名词关键词
  const nouns = ["狗狗", "猫咪", "宠物", "领导", "老板", "同事", "朋友", "家人", "父母", "孩子", "作业", "任务", "项目", "报告", "计划", "会议", "电话", "邮件", "文件", "代码", "设计", "产品", "客户", "用户", "饭", "菜", "水", "茶", "咖啡", "早餐", "午餐", "晚餐", "衣服", "鞋子", "包", "车", "房子", "电脑", "手机"];
  ...
  if (keywords.length === 0) {
    keywords.push(input.substring(0, Math.min(15, input.length)));
  }
...
export async function generatePraiseWithDeepSeek(
  ...
  console.log("[DeepSeek Client] Called with:", { nickname, themeName, userInput });
  console.log("[DeepSeek Client] API Key configured:", !!apiKey);
...
直接输出夸奖内容，不要任何解释或标记。`;
  ...
  if (userInput && userInput.trim()) {
    ...
    console.log("[DeepSeek Client] Extracted keywords:", keywords);
...
第三句：给予温暖和力量`;
  ...
  try {
    ...
    console.log("[DeepSeek Client] Response:", content);
